...
```{r summary color-match-acc}
summary(bglm_color_match_acc)
```

```{r summary color-rdk-acc}
summary(bglm_color_rdk_acc)
```
...
ggplot(df_color_rdk, aes(x = difficulty, y = acc_diff_so, group = subj_idx)) +
  geom_line(alpha = 0.5) +
  stat_summary(aes(group = 1), fun = mean, geom = "line", color = "red", size = 1) + labs(title = "SPE Across Difficulty Levels(Color rdk task ACC)") +
...
```{r bayes glm color-match-rt}
bglm_color_match_rt =  brm(
  bf(
    rt    ~ association * difficulty + (1|subj_idx),  # μ 子模型
    sigma ~ association * difficulty + (1|subj_idx)   # σ 子模型
  ),
  data = data_color_match,
...
summary(bglm_color_match_rt)
```

```{r bayes glm color_rdk-rt}
bglm_color_rdk_rt =  brm(
  bf(
    rt    ~ association * difficulty + (1|subj_idx),  # μ 子模型
    sigma ~ association * difficulty + (1|subj_idx)   # σ 子模型
  ),
...
```{r summary color-rdk-rt}
summary(bglm_color_rdk_rt)
```
...
```{r summary motion-match-acc}
summary(bglm_motion_match_acc)
```

```{r summary motion-rdk-acc}
summary(bglm_motion_rdk_acc)
```
...
ggplot(df_motion_rdk, aes(x = difficulty, y = acc_diff_so, group = subj_idx)) +
  geom_line(alpha = 0.5) +
  stat_summary(aes(group = 1), fun = mean, geom = "line", color = "red", size = 1) + labs(title = "SPE Across Difficulty Levels(Motion rdk task ACC)") +
  papaja::theme_apa(base_size = 18, # base_family = "Times"
                    )
```
...
```{r summary motion-match-rt}
summary(bglm_motion_match_rt)
```


```{r summary motion-rdk-rt}
summary(bglm_motion_rdk_rt)
```
