...
```{r traceplot}
mcmc_trace(m1_color_match_rt, pars = c("b_Intercept", "b_associationother", "b_matchnessmismatch","b_associationother:matchnessmismatch","b_difficulty.L", "sigma")) + papaja::theme_apa()
```
...
df_rt_m1_pop_mean <- m1_color_match_rt %>%
...
# 提取固定效应后验分布
df_rt_m1_pop <- m1_color_match_rt %>% 
        tidybayes::gather_draws(`b_.*`, regex = TRUE) %>%
...
p_rt1 <- df_m1_plot_rt %>%
        ...
        dplyr::mutate(association = factor(association, levels = c('self', 'other'))) %>%
        dplyr::filter(matchness == 'M') %>%
        ggplot2::ggplot(aes(x = value, color = association, fill = association)) +
        geom_density(alpha = 0.7) +
        # stat_halfeye(aes(fill = association), alpha = 0.7) + # 这个包用不了了，不知道为什么
...
# 计算固定效应均值与HDI，这部分summary里有
df_acc_m2_pop_mean <- m2_color_match_acc %>%
        tidybayes::gather_draws(`b_.*`, regex = TRUE) %>%
        group_by(.variable) %>%       # this line not necessary (done automatically by spread_draws)
        tidybayes::mean_hdci(.value)  # get the high density continuous intervals

# 提取固定效应后验分布
df_acc_m2_pop <- m2_color_match_acc %>% 
        tidybayes::gather_draws(`b_.*`, regex = TRUE) %>%
        dplyr::rename(term = .variable,
                      pop_mean = .value) %>%
        #tidyr::separate(term, c(NA, 'term'), "_") 
...
#### 模型拟合：使用默认先验分布
```{r warning=FALSE}
# data_color_rdk <- data_color_rdk %>% dplyr::rename(color = dot_color_final) %>%  # 重命名列
  # mutate(color = recode(               # 替换color值
    # color,
    # '["hsl(225, 50%, 50%)","hsl(0, 50%, 50%)"]' = "blue",  # 匹配→替换
    # '["hsl(0, 50%, 50%)","hsl(225, 50%, 50%)"]' = "red",   # 匹配→替换
  # ))
# iter=1000,Total execution time: 852.2 seconds..18mins
m3_color_rdk_rt <- data_color_rdk %>%
  dplyr::filter(correct == 1) %>%
  brms::brm(
    rt ~ color*association + (1 + color*association|subj_idx),
    data = .,  # 模型设定随实验设计变化
    family=lognormal(),
    chains = 4,   
    warmup = 500,  
...
#### 后验预测
```{r}
pp_check(m3_color_rdk_rt, type = "stat_grouped", group = "color")
```
...
# 计算固定效应均值与HDI，这部分summary里有
df_rt_m3_pop_mean <- m3_color_rdk_rt %>%
        tidybayes::gather_draws(`b_.*`, regex = TRUE) %>%
        group_by(.variable) %>%       # this line not necessary (done automatically by spread_draws)
        tidybayes::mean_hdci(.value)  # get the high density continuous intervals

# 提取固定效应后验分布
df_rt_m3_pop <- m3_color_rdk_rt %>% 
        tidybayes::gather_draws(`b_.*`, regex = TRUE) %>%
        dplyr::rename(term = .variable,
                      pop_mean = .value) %>%
        #tidyr::separate(term, c(NA, 'term'), "_") 
...
m4_color_rdk_acc <- data_color_rdk %>%
  # dplyr::mutate(correct = ifelse(correct == "true", 1, 0)) %>%
  brms::brm(
    correct ~ color + difficulty + (1 + color + difficulty|subj_idx),
    data = .,  # 模型设定随实验设计变化
    family = "bernoulli",
    chains = 4,   
    warmup = 500,  
...
#### 后验预测
```{r}
pp_check(m4_color_rdk_acc, type = "stat_grouped", group = "color")
```
...
# 计算固定效应均值与HDI，这部分summary里有
df_acc_m4_pop_mean <- m4_color_rdk_acc %>%
        tidybayes::gather_draws(`b_.*`, regex = TRUE) %>%
        group_by(.variable) %>%       # this line not necessary (done automatically by spread_draws)
        tidybayes::mean_hdci(.value)  # get the high density continuous intervals

# 提取固定效应后验分布
df_acc_m4_pop <- m4_color_rdk_acc %>% 
        tidybayes::gather_draws(`b_.*`, regex = TRUE) %>%
        dplyr::rename(term = .variable,
                      pop_mean = .value) %>%
        #tidyr::separate(term, c(NA, 'term'), "_") 
...
m8_color_rdk_acc <- df_color_rdk %>%
  brms::brm(
    acc_diff_so ~ difficulty + (1 + difficultyS|subj_idx),
    data = .,  # 模型设定随实验设计变化
    # family=lognormal(),
    family = brmsfamily("gaussian", link = "identity", link_sigma = "log"),
    chains = 4,   
