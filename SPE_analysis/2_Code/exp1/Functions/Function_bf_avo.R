bf_aov <- function(data, dv='acc') {
  ...
  bfs_a_bf10 <- base::rep(1, length(subj_num))
  bfs_a_bf01 <- base::rep(1, length(subj_num))
  ...
  for (i in base::seq_along(subj_num)) {
    ...
    bfs_a_bf10[i] <- BayesFactor::extractBF(model_main / model_difficulty)$bf
    bfs_a_bf01[i] <- BayesFactor::extractBF(model_difficulty / model_main)$bf
    bfs_diff[i] <- BayesFactor::extractBF(model_main / model_association)$bf
  }
  
  aov_output <- tibble::tibble(
    bfs_a_bf10 = bfs_a_bf10,
    bfs_a_bf01 = bfs_a_bf01,
    bfs_diff = bfs_diff,
    bfs_int = bfs_int
  )
